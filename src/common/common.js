...
globalThis.mixreturn = function(dst, src) {
    ...
    if (!Object.hasOwnProperty(dst.is)) {
        dst.is = function(compare) {
            if (compare === dst) {
                return true;
            }
        };
    ...
    if (!Object.hasOwnProperty(src.is)) {
        src.is = function(compare) {
            if (compare === src) {
                return true;
            }
        };
    ...
    for (const [x, func] of src.getMethods()) {

        if (!dst.hasOwnProperty(x)) {//if doesn't exists, simply copy it
            dst.addMethod(x, func);
            continue;
        }

        const overridden = dst[x];

        const prop = function() {

            let r = func.apply(dst, arguments);
            ...
            if (typeof r?.then === 'function') {
                ...
                return new Promise(async (resolve, reject) => {
                    try {
                        ...
                        if (r === undefined) {
                            const r = overridden.apply(dst, arguments);
                            if (typeof r?.then === 'function') {
                                resolve(await r);
                            }
                            resolve(r);
    ...
    Object.ensureProperty(src, '__mixedInto', []).push(dst);

    propagate(dst, dst);

    function propagate(dst, src) {

        if (dst.__mixedInto?.length) {
            for (const base of dst.__mixedInto) {
                copyInto(base, src);
                propagate(base, src);
            }
        }
...
if (typeof module !== 'undefined') {
    module.exports = {mixreturn, is_, parseArgs};
}
